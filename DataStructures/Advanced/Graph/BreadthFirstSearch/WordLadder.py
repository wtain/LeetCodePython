...
#         # toVisit = [0]
#         toVisit = []
#         heapq.heappush(toVisit, (0, 0))
#         while toVisit:
#             # node = toVisit.pop()
#             dist, node = heapq.heappop(toVisit)
#             # visited.append(node)
#             for i in range(n+1):
#                 if M[node][i] != 0 and i not in visited:
#                 # if M[node][i] == 1 and i not in visited:
#                     if distances[i] == 0 or distances[i] > distances[node] + 1:
#                         distances[i] = distances[node] + 1
#                         if i == target+1:
#                             return distances[i] + 1
#                         M[i][node] = M[node][i] = distances[i]
#                     # if i not in toVisit:
#                         # toVisit.append(i)
#                     if i not in visited:
#                         heapq.heappush(toVisit, (distances[i], i))
#                         visited.append(i)
#         return distances[target + 1] + 1 if distances[target + 1] else 0
...
#                 if cnt == 1:
#                     M[i+1][j+1] = M[j+1][i+1] = 1
#         def wordDist(w1, w2: str) -> int:
#             r = 0
#             for i in range(len(w1)):
#                 r += abs(ord(w1[i]) - ord(w2[i]))
#             return r
#         distances = [0] * (n+1)
#         visited = [0]
#         toVisit = []
#         heapq.heappush(toVisit, (wordDist(beginWord, endWord), 0))
#         while toVisit:
#             dist, node = heapq.heappop(toVisit)
#             for i in range(n+1):
#                 if M[node][i] != 0 and i not in visited:
...
#                         M[i][node] = M[node][i] = distances[i]
#                     if i not in visited:
#                         heapq.heappush(toVisit, (wordDist(wordList[i-1], endWord), i))
#                         visited.append(i)
#         return distances[target + 1] + 1 if distances[target + 1] else 0

# Runtime: 1112 ms, faster than 10.18% of Python3 online submissions for Word Ladder.
# Memory Usage: 17.6 MB, less than 40.63% of Python3 online submissions for Word Ladder.
# class Solution:
#     def ladderLength(self, beginWord: str, endWord: str, wordList: List[str]) -> int:
#         if endWord not in wordList:
#             return 0
...
#             for i in range(L):
#                 int_w = w[:i] + "*" + w[i+1:]
#                 allWords[int_w].append(w)
#         toVisit = collections.deque([(beginWord, 1)])
#         visited = [beginWord]
#         while toVisit:
#             cw, dist = toVisit.popleft()
#             for i in range(L):
#                 int_w = cw[:i] + "*" + cw[i + 1:]
#                 for w in allWords[int_w]:
#                     if w == endWord:
#                         return dist + 1
#                     if w not in visited:
#                         visited.append(w)
#                         toVisit.append((w, dist+1))
#                 allWords[int_w] = []
#         return 0

# Was wrong
# Runtime: 92 ms, faster than 96.60% of Python3 online submissions for Word Ladder.
# Memory Usage: 17.6 MB, less than 45.80% of Python3 online submissions for Word Ladder.
# class Solution:
#
#     def ladderLength(self, beginWord: str, endWord: str, wordList: List[str]) -> int:
#         if endWord not in wordList:
...
#                 int_w = w[:i] + "*" + w[i+1:]
#                 self.allWords[int_w].append(w)
#
#         def visitNode(toVisit, visited1, visited2) -> int:
#             cw, dist = toVisit.popleft()
#             for i in range(self.L):
#                 int_w = cw[:i] + "*" + cw[i + 1:]
#                 for w in self.allWords[int_w]:
#                     if w in visited2:
#                         return dist + visited2[w]
#                     if w not in visited1:
#                         visited1[w] = dist + 1
#                         toVisit.append((w, dist+1))
#             return 0
#
#         toVisit1 = collections.deque([(beginWord, 1)])
#         toVisit2 = collections.deque([(endWord, 1)])
#         visited1 = {beginWord: 1}
#         visited2 = {endWord: 1}
#         while toVisit1 and toVisit2:
#             result = visitNode(toVisit1, visited1, visited2)
#             if result:
#                 return result
#             result = visitNode(toVisit2, visited2, visited1)
#             if result:
#                 return result
#         return 0
...
# Runtime: 145 ms, faster than 80.74% of Python3 online submissions for Word Ladder.
# Memory Usage: 18.4 MB, less than 16.31% of Python3 online submissions for Word Ladder.
class Solution:
    ...
    def ladderLength(self, beginWord: str, endWord: str, wordList: List[str]) -> int:
        ...
        visited1 = {beginWord: 1}
        visited2 = {endWord: 1}
        min_result = BIG_VALUE
        while toVisit1 and toVisit2:
            result = visitNode(toVisit1, visited1, visited2)
            min_result = min(min_result, result)
            result = visitNode(toVisit2, visited2, visited1)
            min_result = min(min_result, result)
        if min_result == BIG_VALUE:
            return 0
        return min_result
...
run_functional_tests(Solution().ladderLength, tests)
