...
#             nonlocal result, nodes, labels
#             same = 1 if counter[labels[node]] > 0 else 0
    ...
    def countSubTrees(self, n: int, edges: List[List[int]], labels: str) -> List[int]:
        ...
        def dfs(node: int, parent: int):
            ...
            for child in nodes[node]:
                if child == parent:
                    continue
                subcounter = dfs(child, node)
                for k in subcounter:
                    counter[k] += subcounter[k]
...
run_functional_tests(Solution().countSubTrees, tests)
